...
def eyebrow(origin_src,R,G,B):
  ...
  # file 저장
  cv2.imwrite("./output/eyebrow_modified.PNG", eyebrow_mask)

  if (MAKE_TRANSPARENT):
    img = Image.open("./output/eyebrow_modified.PNG")  # 파일 열기
    img = img.convert("RGBA")  # RGBA형식으로 변환
    ...
    img.save("./output/eyebrow_modified.PNG")
  img = cv2.imread(origin_src)
